...
export function Subheading({
	...
	return (
		<Element
			...
			className={clsx(
				className,
				'flex flex-row items-center gap-2 font-mono text-xs/5 font-semibold uppercase tracking-widest text-gray-500 data-[dark]:text-gray-400',
			)}
		/>
	);
}
...
export function Lead({ className, ...props }: React.ComponentPropsWithoutRef<'p'>) {
	return (
		<p className={clsx(className, 'text-2xl font-medium text-gray-500')} {...props} />
	);
}
