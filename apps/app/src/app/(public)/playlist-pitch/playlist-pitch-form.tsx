		...
		<>
							...
							<>
								...
								{!user && (
									<>
										...
										<UserContactInfoForm
											newUser
											submitLabel='Submit my campaign 🚀'
			...
			{formSubmitStatus === 'submitted' && (
				<div className='flex flex-col items-center max-w-xl mx-auto text-center'>
					...
					<H2>Success!</H2>
					<Text variant='md/normal' className='mt-4'>
						{`We've received your campaign. Our A&R team will review and be in touch soon. In the meantime, check your email for a confirmation and link to check your campaign's status.`}
					</Text>
				</div>
			)}
		</>
...
const CheckSelectedArtist = (props: { artistSpotifyId?: string | null }) => {
	...
	// return (
	// 	<>
	// 		<p>artistSpotifyId to check: {props.artistSpotifyId}</p>
	// 		<p>selected artist: {JSON.stringify(artist, null, 2)}</p>
	// 	</>
	// );
};
