...
import {
	...
	CuratorReachInput,
} from '~/app/(dash)/campaigns/components/curator-reach-input';
import {
	minReachAtom,
...
const LaunchCampaignForm = ({ initialCampaign }: LaunchCampaignProps) => {
	...
	return (
		<Form formAtom={launchCampaignFormAtom} onSubmit={() => console.log('submitting')}>
			...
			<div className='flex flex-row gap-3 pt-4 pb-3'>
				<Badge rectangle size='lg' variant='outline' grow>
					...
					<Suspense fallback={<span>...</span>}>
						<span>
							{isNaN(estimatedPlaylists) ? 'boo' : estimatedPlaylists} playlists
						</span>
					</Suspense>
				</Badge>
				<Badge rectangle size='lg' variant='outline' grow>
			...
			<Button loading={creatingCheckoutSession} onClick={handleCheckout} variant='go'>
				Launch my campaign 🚀
			</Button>
		</Form>
	);
