...
export function CreateOrUpdateMixtapeModal(props: { mode: "create" | "edit" }) {
  ...
  const { mutateAsync: insertTracks } = api.mixtape.insertTracks.useMutation({
    async onMutate({ mixtapeId, _tracks }) {
      ...
      const newTracks = _tracks
        ...
        .filter(
          (_t) => _t !== undefined,
        ) as (typeof newMixtapeTracks)[number][];
  ...
  const { mutateAsync: removeTracks } = api.mixtape.removeTracks.useMutation({
    async onMutate(mixtape) {
      ...
      const previousTracks =
        apiUtils.mixtape.byWorkspace
          .getData()
          ?.find((m) => m.id === mixtape.mixtapeId)?.tracks ?? [];

      const updatedTracks = previousTracks.filter(
        (t) => !mixtape.trackIds.includes(t.id),
      );
    ...
    async (trackIds: string[]) => {
      if (mode === "create") {
        setNewMixtapeTracks((prev) =>
          prev.filter((t) => !trackIds.includes(t.id)),
        );
  ...
  const { form, onSubmit: onSubmitMixtape } = useCreateOrUpdateForm({
    editItem: mode === "create" ? null : selectedMixtape ?? null,
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                {(track) => (
                  <MixtapeTrackCard
                    track={track}
                    handleRemove={handleRemoveTracks}
                  />
                )}
              </GridList>
            </div>
