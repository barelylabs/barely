import type { InsertEmailDomain } from '@barely/lib/server/routes/email/email.schema';
...
import { createEmailDomainSchema } from '@barely/lib/server/routes/email/email.schema';
...
export function EmailDomainModal() {
	...
	const { mutateAsync: createEmailDomain } = api.email.createDomain.useMutation({
		onSuccess: async () => {
	...
	const { mutateAsync: updateEmailDomain } = api.email.updateDomain.useMutation({
		onSuccess: async () => {
			await apiUtils.email.invalidate();
			setShowEmailDomainModal(false);
			setEditEmailDomain(undefined);
