...
import type {
  ...
  linkSelectionParamsSchema,
} from "@barely/lib/server/link.schema";
...
export function LinkContextProvider({
  ...
  filters: z.infer<typeof linkFilterParamsSchema>;
  selectedLinkIds: z.infer<typeof linkSelectionParamsSchema> | undefined;
}) {
  ...
  const { data, setQuery, removeByKey, removeAllQueryParams } = useTypedQuery(
    linkSearchParamsSchema,
  );

  /* selection */
  const [optimisticSelection, setOptimisticSelection] =
    useOptimistic<Selection>(new Set(selectedLinkIds));
