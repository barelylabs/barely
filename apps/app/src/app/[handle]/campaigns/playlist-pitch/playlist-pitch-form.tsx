...
export function PlaylistPitchForm(props: { user?: SessionUser }) {
  ...
  const { data: spotifyTrackOptions, isFetching } =
    api.spotify.findTrack.useQuery(trackQ, {
      enabled: trackQ.length > 0,
      placeholderData: keepPreviousData,
    });

  const [selectedTrack, setSelectedTrack] = useState<SpotifyTrackOption | null>(
    null,
  );
  ...
  const { data: dbArtist, isFetching: _checkingDbArtist } =
    api.workspace.bySpotifyId.useQuery(
  ...
  const _existingUserCanAccessArtist =
    ...
    props.user?.workspaces.some(
      (workspace) =>
        workspace.spotifyArtistId === selectedTrack?.workspace.spotifyArtistId,
    );
  ...
  if (submitStatus === "idle")
    return (
      <>
        ...
        {(_newUserCannotAccessArtist ?? _existingUserCannotAccessArtist) &&
          dbArtist && (
            <Text variant="sm/normal" className="mt-4">
              It looks like {dbArtist.name ?? "That artist"} is already in our
              database. Please{" "}
              {_newUserCannotAccessArtist && (
                <span className="underline hover:text-purple">
                  <Link
                    href={`/login?redirect=${encodeURIComponent(
                      `/campaigns/new?type=playlistPitch&artistId=${
                        dbArtist.id
                      }&${
                        dbTrack
                          ? "trackId=" + dbTrack.id
              ...
              {_existingUserCannotAccessArtist && (
                <span>
                  request access from the artist to submit a new campaign.
                </span>
              )}
            </Text>
          )}
        ...
        {!props.user && (
          <Text variant="sm/normal" className="mt-4">
            <span className="bg-yellow-50">
              In the meantime, check your email for a confirmation and link to
              check your campaign&apos;s status.
            </span>
          </Text>
        )}
      </div>
