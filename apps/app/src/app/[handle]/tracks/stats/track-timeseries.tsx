...
function SingleTrackStats({
	...
	const processedTimeseries = timeseries
		...
		.map(row => ({
			...
			timestamp: new Date(row.timestamp ?? raise('timestamp is required')),
		}))
...
function MultiTrackComparison({
	...
	const { chartData, maxValue } = useMemo(() => {
		const dateMap = new Map<
			string,
			{ date: string; timestamp: Date; [key: string]: number | string | Date }
		>();
		let max = 0;
		...
		trackData.forEach(track => {
			track.timeseries.forEach(row => {
				if (row.timestamp) {
					...
					if (!dateMap.has(date)) {
						dateMap.set(date, { date, timestamp: new Date(row.timestamp) });
					}
				}
			});
		});
