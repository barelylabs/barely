...
export function EnhancedSuccessTicker() {
  ...
  return (
    <div 
      ref={containerRef}
      className="inline-flex flex-col items-center gap-3 px-6 py-3 rounded-full bg-white/5 border border-white/10 transition-all duration-500 ease-out"
      style={{ width }}
    >
      <div className="flex items-center gap-4">
        ...
        )} aria-hidden="true">{current.icon}</span>
        ...
        {successStories.map((_, index) => (
          <div
            ...
            className={cn(
              ...
              index === currentIndex ? 'bg-green-500 w-4' : 'bg-white/20'
            )}
          />
        ))}
      </div>
