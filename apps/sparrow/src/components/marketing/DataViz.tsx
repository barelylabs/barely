...
export function StatCard({
	...
	return (
		<div ref={ref} className={cn('glass rounded-xl p-6 text-center', className)}>
			...
			<p className={cn('text-4xl font-bold', color)}>{formattedCount}</p>
		</div>
	);
}
		...
		<div ref={ref} className={cn('relative', className)} style={{ height }}>
            ...
            ${data
							.map((point, i) => {
								...
								const y =
									height -
									((point.value - minValue) / range) * height * 0.8 -
									height * 0.1;
          ...
          `}
				...
				{data.map((point, i) => {
					const x = (i / (data.length - 1)) * 100;
					const y =
						height - ((point.value - minValue) / range) * height * 0.8 - height * 0.1;
					return (
						<circle
							key={i}
							cx={`${x}%`}
			...
			{/* Labels */}
			<div className='absolute bottom-0 left-0 right-0 mt-2 flex justify-between text-xs text-white/50'>
				{data
