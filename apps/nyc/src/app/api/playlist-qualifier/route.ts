...
import { PlaylistQualifierEmail } from '@barely/email/templates/nyc/playlist-qualifier';
...
export async function POST(request: Request) {
	try {
		// Validate input
		const validatedData = playlistQualifierSchema.parse(await request.json());
		...
		// Send email notification with qualifier information
		const emailResult = await sendEmail({
			from: 'noreply@mail.barely.nyc',
			fromFriendlyName: '@barely.indie',
			to: 'hello@barely.nyc',
			subject: `🎯 Qualified Lead - ${validatedData.artistName} (${validatedData.budgetRange})`,
			react: PlaylistQualifierEmail({
		...
		console.error('Playlist qualifier error:', error);
