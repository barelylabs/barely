...
import { z } from 'zod';
import { linkBaseSchema } from '@barely/db/zod/link';
import { appTypeSchema } from '@barely/db/zod/apptype';
		...
		.query(async ({ ctx, input }) => {
			...
			const link = await ctx.prisma.link.findFirst({
				...
				select: {
					id: true,
					url: true,
					artist: { select: { remarketing: { select: { remarketing: true } } } },
			...
			const remarketingEndpoints = link.artist?.remarketing.map(r => r.remarketing) ?? [];
			const { artist, ...linkWithoutArtist } = link;
			const linkWithRemarketing = { ...linkWithoutArtist, remarketingEndpoints };
