import {
	...
	storyCreateSchema,
	storyUpdateSchema,
} from '@barely/schema/db';
import { getLexoRank } from '@barely/utils';
...
export const storyBoardRouter = router({
	add: privateProcedure.input(storyBoardCreateSchema).mutation(async ({ ctx, input }) => {
		return await ctx.prisma.storyBoard.create({
			data: {
				name: input.name,
	...
	byUser: privateProcedure.query(async ({ ctx }) => {
		return await ctx.prisma.storyBoard.findMany({
			take: 20,
			where: {
				OR: [
...
export const storyColumnRouter = router({
	add: privateProcedure
		...
		.mutation(async ({ ctx, input }) => {
			const lastColumn = await ctx.prisma.storyColumn.findFirst({
			...
			const rank = lastColumn
				...
				: getLexoRank.middle();
			return await ctx.prisma.storyColumn.create({
				data: {
	...
	update: privateProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.lexo.updateRank) {
				const betweenColumns = await ctx.prisma.storyColumn.findMany({
					where: {
						OR: [
			...
			return await ctx.prisma.storyColumn.update({
				where: { id: input.id },
	...
	byBoard: privateProcedure
		...
		.query(async ({ ctx, input }) => {
			return await ctx.prisma.storyColumn.findMany({
	...
	add: privateProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.lexo.position === 'first') {
				const firstStory = await ctx.prisma.story.findFirst({
					where: { columnId: input.columnId },
					select: { lexoRank: true },
					orderBy: { lexoRank: 'asc' },
				});
			...
			if (input.lexo.position === 'last') {
				const lastStory = await ctx.prisma.story.findFirst({
					where: { columnId: input.columnId },
					select: { lexoRank: true },
					orderBy: { lexoRank: 'desc' },
				});
			...
			if (input.lexo.position === 'between') {
				const betweenStories = await ctx.prisma.story.findMany({
					where: {
						OR: [
