...
// 	NestedLexicalEditor,
// 	useMdastNodeUpdater,
// } from '../plugins/core/NestedLexicalEditor';
...
const isMdxJsxAttribute = (
	value: MdxJsxAttribute | MdxJsxExpressionAttribute,
): value is MdxJsxAttribute => {
...
export const GenericJsxEditor: React.FC<JsxEditorProps> = ({ mdastNode, descriptor }) => {
	...
	const onChange = React.useCallback(
		(values: Record<string, string>) => {
			const updatedAttributes = Object.entries(values).reduce<
				typeof mdastNode.attributes
			>((acc, [name, value]) => {
				if (value === '') {
					return acc;
				}
	...
	const shouldRenderComponentName =
		descriptor.props.length == 0 && descriptor.hasChildren && descriptor.kind === 'flow';

	return (
		<div
			className={descriptor.kind === 'text' ? styles.inlineEditor : styles.blockEditor}
		>
			{shouldRenderComponentName ?
				<span className={styles.genericComponentName}>
