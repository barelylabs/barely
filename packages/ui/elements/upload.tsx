...
import type {
  UploadQueueItem,
  UseUploadReturn,
} from "@barely/lib/hooks/use-upload";
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "group relative z-[3] mt-1 flex h-64 w-64 cursor-pointer flex-col items-center justify-center overflow-hidden rounded-lg border border-border shadow-sm transition-all",
          className,
        )}
      >
        <div
    ...
    >
      <div className="flex w-full max-w-full flex-row items-center justify-between border-b border-border bg-slate-50 p-2">
        <Button
          look="minimal"
          size="sm"
          onClick={() => uploadState.clearUploadQueue()}
        >
          Cancel
        </Button>

        <Text variant="sm/semibold" muted>
          {nFormatter(uploadState.uploadQueue.length, { digits: 0 })} files
          selected
        </Text>
