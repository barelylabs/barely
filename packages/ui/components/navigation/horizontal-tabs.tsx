      ...
      >
        {tabs.map((tab, idx) => (
          <HorizontalTabItem
            ...
            leftHref={
              idx > 0 ? tabs[idx - 1]?.href : tabs[tabs.length - 1]?.href
            }
            rightHref={
              idx < tabs.length - 1 ? tabs[idx + 1]?.href : tabs[0]?.href
            }
          />
        ))}
      </nav>
...
export function HorizontalTabItem({
  ...
  const onLeftKeyDown = useCallback(() => {
    if (isCurrent && leftHref && elementIsFocused(linkRef))
