...
export function useSnapPoints({
		...
		() =>
			snapPoints?.map(snapPoint => {
				...
				if (isVertical(direction)) {
					const height =
						isPx ? snapPointAsNumber
						: hasWindow ? snapPoint * window.innerHeight
						: 0;
				...
				const width =
					isPx ? snapPointAsNumber
					: hasWindow ? snapPoint * window.innerWidth
					: 0;
	...
	const activeSnapPointOffset = React.useMemo(
		() => (activeSnapPointIndex ? snapPointsOffset?.[activeSnapPointIndex] : null),
	...
	React.useEffect(() => {
		if (activeSnapPointProp) {
			const newIndex =
				snapPoints?.findIndex(snapPoint => snapPoint === activeSnapPointProp) ?? -1;
			if (
				snapPointsOffset &&
				newIndex !== -1 &&
	...
	function getPercentageDragged(absDraggedDistance: number, isDraggingDown: boolean) {
		...
		const targetSnapPointIndex =
			isOverlaySnapPoint ? activeSnapPointIndex + 1 : activeSnapPointIndex - 1;

		// Get the distance from overlaySnapPoint to the one before or vice-versa to calculate the opacity percentage accordingly
		const snapPointDistance =
			isOverlaySnapPoint ?
