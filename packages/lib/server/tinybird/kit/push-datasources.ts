...
async function createDataSource(name: string, schema: string): Promise<unknown> {
	...
	if (!process.env.TINYBIRD_API_KEY) return console.log('no tinybird api key found');
	...
	.then(files => {
		const promises = files
			...
			.map(file => {
				...
				const schemaSection = fileContent.split('SCHEMA >')[1]?.split(/\n{2,}/)[0];
				...
				const schema =
					schemaLines
						?.map(line => {
							...
							return `${name?.replaceAll('`', '')} ${type} ${path?.split(',')[0]}`;
