  ...
  >(req: {
    ...
    return async (events: TInput | TInput[]) => {
      ...
      if (res.status === 429) {
        ...
        await new Promise((r) =>
          setTimeout(r, retryAfter ? parseInt(retryAfter) : 1000),
        );
        res = await fetch(url, {
          method: "POST",
          body,
      ...
      if (!res.ok) {
        throw new Error(
          `Unable to ingest to ${req.datasource}: [${
            res.status
          }] ${await res.text()}`,
        );
      }

      const validatedResponse = eventIngestReponseData.safeParse(
        await res.json(),
      );
