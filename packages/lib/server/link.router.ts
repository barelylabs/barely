...
import { getAppAndAppRouteFromUrl, getMetaTags, isValidUrl } from '../utils/link';
...
export const linkRouter = router({
	...
	create: privateProcedure.input(createLinkSchema).mutation(async ({ input, ctx }) => {
		...
		const link = await ctx.db.write.insert(Links).values(createLinkValues).returning();
		return link;
	}),
	...
	update: privateProcedure.input(updateLinkSchema).mutation(async ({ input, ctx }) => {
		...
		const url =
			input.url ??
			(await ctx.db.read.query.Links.findFirst({ where: eq(Links.id, input.id) }))?.url ??
	...
	getMetaTags: publicProcedure.input(z.string()).query(async ({ input: url, ctx }) => {
		if (!url || !url.length || !isValidUrl(url)) {
			throw new TRPCError({
				code: 'BAD_REQUEST',
				message: `Invalid URL`,
			});
