...
export async function createLoginLink(props: {
	...
	return `${env.AUTH_URL}/api/auth/callback/${props.provider}?${params.toString()}`;
}
...
export async function deleteSession(sessionToken: string) {
	...
	await db.write.delete(UserSessions).where(eq(UserSessions.sessionToken, sessionToken));
}
