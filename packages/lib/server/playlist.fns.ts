...
const getPlaylistById = async (playlistId: string, db: Db) => {
  ...
  const playlistWithProviderAccounts: PlaylistWithProviderAccounts = {
    ...
    providerAccounts: playlist._providerAccounts.map(
    ...
    await Promise.allSettled(
      genreIds.map(async (genreId) => {
        await tx
          ...
          .onConflictDoNothing({
            target: [
              _Playlists_To_Genres.playlistId,
...
type TotalPlaylistReach = inferAsyncReturnType<
  typeof totalPlaylistReachByGenres
>;
