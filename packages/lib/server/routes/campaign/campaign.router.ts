...
import { createTrack } from '../track/track.fns';
import { Tracks } from '../track/track.sql';
import { createUser, getSessionUserByUserId } from '../user/user.fns';
import { _Users_To_Workspaces } from '../user/user.sql';
import { createWorkspace } from '../workspace/workspace.fns';
...
export const campaignRouter = createTRPCRouter({
	...
	createUserAndPlaylistPitch: publicProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			await ctx.db.pool.transaction(async tx => {
				...
				// 💿 create new track
				newTrack =
					(await createTrack(input.track, newWorkspace.id, ctx.db, tx)) ?? undefined;
			});
	...
	byWorkspaceId: privateProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const campaigns = await getCampaignsByWorkspaceId(input.workspaceId, ctx.db, {
				type: input.type,
				stage: input.stage,
				limit,
			});
	...
	submitPlaylistPitchScreening: privateProcedure
		...
		.mutation(async ({ ctx, input: rawInput }) => {
			...
			await ctx.db.http.update(Campaigns).set(input).where(eq(Campaigns.id, input.id));
