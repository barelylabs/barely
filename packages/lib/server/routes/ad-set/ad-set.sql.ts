...
import { AdCampaigns } from '../ad-campaign/ad-campaign.sql';
import { Ads } from '../ad/ad.sql';
import { Audiences } from '../audience/audience.sql';
...
export const AdSetCloneRecords = pgTable(
	...
	table => ({
		...
		adSetParent: index('AdSetCloneRecord_adSetParent_idx').on(table.adSetParentId),
		adSetChild: index('AdSetCloneRecord_adSetChild_idx').on(table.adSetChildId),
	}),
);

export const AdSetCloneRecord_Relations = relations(AdSetCloneRecords, ({ one }) => ({
	// one-to-one
...
export const AdSetUpdateRecord_Relations = relations(AdSetUpdateRecords, ({ one }) => ({
	// one-to-many
	adSet: one(AdSets, {
