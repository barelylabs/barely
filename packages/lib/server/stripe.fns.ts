...
// export function getStripeConnectDataFromWorkspace(workspace: SessionWorkspace) {
...
export async function createStripeWorkspaceCustomer(props: {
	...
	// update workspace
	const stripeWorkspace = await props.db.http
		.update(Workspaces)
		.set(
			env.VERCEL_ENV === 'development' || env.VERCEL_ENV === 'preview'
				? {
...
// }) {
//   let stripeConnectAccountId = workspace.stripeConnectAccountId;

//   if (!stripeConnectAccountId) {
//     const account = await stripe.accounts.create({
//       type: "standard",
//       // todo: prefill with workspace info
//     });
...
//     refresh_url: "", // todo: this should be a page that calls our api to generate another onboarding link, returns it to the page, and then the page redirects to the onboardin link
//     return_url: getAbsoluteUrl("app", callbackPath),
	...
	const paymentIntent = await stripe.paymentIntents.create(
		{
			...
			application_fee_amount: 100, // 1 dollar // todo: this should be a percentage of the total
		},
		{
			stripeAccount: 'acct_1Jr5fj2eZvKYlo2C', // fixme
		},
	);
...
export async function updateCartPaymentIntent({
	paymentIntentId,
}: {
