...
import { createPlaylistPlacementSchema } from "./playlist-placement.schema";

export const insertPlaylistPitchReviewSchema =
  createInsertSchema(PlaylistPitchReviews);
export const createPlaylistPitchReviewSchema =
  insertPlaylistPitchReviewSchema.omit({
    id: true,
  });
export const selectPlaylistPitchReviewSchema =
  createSelectSchema(PlaylistPitchReviews);

export type PlaylistPitchReview = InferSelectModel<typeof PlaylistPitchReviews>;
export type InsertPlaylistPitchReview = z.infer<
  typeof insertPlaylistPitchReviewSchema
>;
export type CreatePlaylistPitchReview = z.infer<
  typeof createPlaylistPitchReviewSchema
>;
...
export const submitPlaylistPitchReviewSchema = insertPlaylistPitchReviewSchema
  ...
  .extend({
    ...
    placements: z.array(
      createPlaylistPlacementSchema.extend({ place: z.boolean() }),
    ),
  })
  .refine((review) => {
    if (
