...
export function tFormatter(seconds?: number, opts: { showHours?: boolean } = {}) {
	...
	const formattedSeconds = secs < 10 ? `0${secs.toFixed(0)}` : `${secs.toFixed(0)}`;

	// Determine if hours should be shown
	const showHours = (hours > 0 && opts.showHours !== false) || opts.showHours === true;
