...
import { linkSchema } from '../../api/link/link.schema';
import { undefinedToNull } from '../../utils/edge/handle-undefined';

const getDeviceData = (req: NextRequest) => {
...
const getPathParams = (req: NextRequest) => {
	...
	const parsedApp = linkSchema.shape.app.safeParse(appString);
	const app = parsedApp.success ? parsedApp.data : null;
	console.log('parsedApp => ', app);

	const appRoute = undefinedToNull(appRouteString);
	const appId = undefinedToNull(appIdString);
...
export { getDeviceData, getPathParams };
