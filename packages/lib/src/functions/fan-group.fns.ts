...
import type {
	FanGroupCondition,
	FanGroupWithConditions,
} from '@barely/validators/schemas';
import type { SQL } from 'drizzle-orm';
import { dbPool } from '@barely/db/pool';
import { Carts, FanGroupConditions, FanGroups, Fans } from '@barely/db/sql';
import { sqlAnd, sqlCount } from '@barely/db/utils';
...
export async function getFanGroupById(id: string, pool: NeonPool) {
	const fanGroup = await dbPool(pool).query.FanGroups.findFirst({
...
const getFansWithIdSubquery = (workspaceId: string, pool: NeonPool) =>
	dbPool(pool)
		.select({
			fanId: Fans.id,
		})
...
const getFansForEmailSubquery = (workspaceId: string, pool: NeonPool) =>
	dbPool(pool)
		.select({
			fanId: Fans.id,
			email: Fans.email,
...
const getCartsSubquery = (pool: NeonPool) =>
	dbPool(pool)
		.select({
			// id: Carts.id,
			fanId: Carts.fanId,
...
export async function getFanGroupCount(fanGroup: FanGroupWithConditions, pool: NeonPool) {
	const fansSubquery = getFansWithIdSubquery(fanGroup.workspaceId, pool);
	const cartSubquery = getCartsSubquery(pool);

	const countQuery = dbPool(pool)
		.select({
			count: sqlCount,
		})
		.from(fansSubquery)
...
export async function getFanGroupFansForEmail(
	...
	const fansSubquery = getFansForEmailSubquery(fanGroup.workspaceId, pool);
	const cartSubquery = getCartsSubquery(pool);

	const fansQuery = dbPool(pool)
		.select({
