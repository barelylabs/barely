...
export function useUpload({
  ...
  const [uploading] = useAtom(
    useMemo(
      () =>
        atom((get) =>
          get(uploadQueueAtom).some((q) => q.status === "uploading"),
        ),
      [uploadQueueAtom],
    ),
  );
        ...
        getPresigned(
          ...
          },
          {
            onSuccess: (presigned) => {
              setUploadQueue((prev) =>
                prev.map((q) => {
                  const presignedFile = presigned.find(
                    (pu) => pu.name === q.file.name,
                  );
                  if (!presignedFile) {
                    if (q.presigned === null) {
                      console.error(
                        `No presigned URL found for ${q.file.name}`,
                      );
                    }
                    return q;
                  }
  ...
  const onUploadCompleteCallback: OnUploadComplete = useCallback(
    async (fileRecord) => {
      ...
      setUploadQueue((prev) =>
        prev.map((p) => {
          if (fileRecord.id === p.presigned?.fileId) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (
      ...
      !uploadQueue.some((q) => q.presigned === null)
    ) {
      await Promise.all(
