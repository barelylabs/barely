...
import { cartFiltersSchema } from '../utils/filters';
...
export function useCartStatFilters() {
	...
	const toggleShowEmailAdds = useCallback(() => {
		if (showEmailAdds) return q.setQuery('showEmailAdds', false);
		return q.setQuery('showEmailAdds', true);
	}, [showEmailAdds, q]);

	const toggleShowShippingInfoAdds = useCallback(() => {
		if (showShippingInfoAdds) return q.setQuery('showShippingInfoAdds', false);
		return q.setQuery('showShippingInfoAdds', true);
	}, [showShippingInfoAdds, q]);

	const toggleShowPaymentInfoAdds = useCallback(() => {
		if (showPaymentInfoAdds) return q.setQuery('showPaymentInfoAdds', false);
	...
	const toggleShowMainWithoutBumpPurchases = useCallback(() => {
		if (showMainWithoutBumpPurchases)
			return q.setQuery('showMainWithoutBumpPurchases', false);
